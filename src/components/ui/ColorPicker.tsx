...
export function ColorPicker({
  ...
  const hueRef = useRef<HTMLDivElement>(null); // Load previous colors from localStorage on component mount
  useEffect(() => {
  ...
  const addToPreviousColors = (color: string) => {
    ...
    if (color && color.startsWith("#")) {
      const newPreviousColors = [
        color,
        ...previousColors.filter((c) => c !== color).slice(0, 5),
      ];
      setPreviousColors(newPreviousColors); // Save to localStorage
    ...
    <div className="relative">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          ...
          <Palette
            className={`w-8 h-8 ${
              selectedColor ? "text-white" : "text-gray-500"
            }`}
            style={{
